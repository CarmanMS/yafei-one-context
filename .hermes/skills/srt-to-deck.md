...
**Remotion / Headless Chrome 渲染检查**（若使用帧级渲染方案）：

- [ ] 不使用 `-webkit-text-fill-color: transparent` + `background-clip: text`（Headless Chrome 不渲染，改用纯色 + textShadow）
- [ ] 不使用 `backdrop-filter: blur()`（Headless Chrome 不支持，改用 `rgba()` 半透明背景）
- [ ] 装饰元素 alpha 值 ≥ 0x35（极低 alpha 在 headless 渲染中不可见）
- [ ] 背景基底色明度 ≥ `#111827`（过深底色导致整体发黑）
- [ ] SVG 内文字对标页面字号体系：标题 ≥ 40px、正文 ≥ 32px、标签 ≥ 28px（详见 `exp-remotion/RENDERING_NOTES.md`）
- [ ] SVG 添加 `maxWidth:100%; height:auto` 防止溢出容器
- [ ] 渲染验证使用中间帧（`startFrame + 150`），不用 `frame=0`（Appear 动画初始 opacity=0）
- [ ] 像素亮度检测：内容区平均亮度 ≥ 50、亮像素(>120) ≥ 3%、白色文字 ≥ 1.5%

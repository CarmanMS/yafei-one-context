...
## one-context content-pipeline 默认

新建 `features/content-pipeline/*` 时 **默认 action=0**（`00-podcast-source.md` 长文输入，成片以 WAV+SRT 为准）。**不要**因已写双人 `01-script.md` 就改用 action=3；例外须在 spec 填 `tts.override_reason` 并经 `review_record` 确认。见 **`knowledge/standards/content-pipeline-tts-routing.md`**。

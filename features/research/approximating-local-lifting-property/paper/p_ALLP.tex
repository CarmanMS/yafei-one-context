...
\begin{remark}\label{rem:daws-quotient-dual}
Let $W_0\subseteq W$ be $p$-operator spaces and let $\pi:W\to W/W_0$ be the
$p$-complete quotient map.  By \cite{Daws2007}, $\pi':(W/W_0)^*\to W^*$ is a
$p$-complete isometry with image
\[
        W_0^\perp:=\{\mu\in W^*:\ \langle\mu,w\rangle=0\ \hbox{for all }w\in W_0\}.
\]
In particular, $(W/W_0)^*$ is $p$-completely isometric to the subspace
$W_0^\perp\subseteq W^*$.  Daws also points out that, in general, one should
\emph{not} expect a $p$-complete identification of $W_0^*$ with a quotient of
$W^*$ (nor that the restriction map $W^*\to W_0^*$ is a $p$-complete quotient
map).  The classical proof in \cite{KyeRuan1999} that the $\lambda$-LLP of $V$
implies $\lambda$-injectivity of $V^*$ uses precisely such an identification at
the level of operator duals.  For general $p$-operator spaces this step is
therefore unavailable, and we treat the implication from lifting properties on
$V$ to injectivity of $V^*$ separately below.
\end{remark}
...
\]
does not follow from Remark~\ref{rem:daws-quotient-dual} alone.  When $p=2$,
...
\[
        \CBp(W,V^*) \cong (W \widehat{\otimes}_p V)^*.
\]
Under this identification, the weak$^*$ and point-weak$^*$ topologies on
$\mathcal{K}_M$ coincide, since $W\otimes V$ is dense in $W\widehat{\otimes}_p V$.
...
\[
M_m(V^*)=\CBp(V,\mathbb{M}_m)\cong (\mathcal{N}_m^p\widehat{\otimes}_p V)^*,
\]
...
\section{Injectivity of $B_p(\ell_p)$ and LLP of $T_p(\ell_p)$}

In this section, we apply our framework to the specific case of the $p$-operator space $B_p(\ell_p)$. As indicated by Lee \cite{Lee2015}, the Arveson--Wittstock extension theorem fails for general $p$-operator spaces because there exist $SQ_p$ spaces $E$ such that $B(E)$ is not $p$-injective. However, the $p$-injectivity of $B_p(\ell_p)$ remains a major open problem. We show that for $\ell_p$ spaces, the equivalence between the $p$-injectivity of $B_p(\ell_p)$ and the $p$-local lifting property of $T_p(\ell_p)$ holds by bypassing the general dual quotient map obstructions and utilizing the specific rigid structure of $\ell_p$.

\begin{theorem}\label{thm:lp-equivalence}
Let $1 < p < \infty$. Then $B_p(\ell_p)$ is $p$-injective if and only if $T_p(\ell_p)$ has the $p$-LLP.
\end{theorem}

\begin{proof}
\textbf{Direction 1: $B_p(\ell_p)$ is $p$-injective $\Rightarrow T_p(\ell_p)$ has the $p$-LLP.}
Let $W\subseteq Y$ be $p$-operator spaces, $q:Y\to Y/W$ be the $p$-complete quotient map, and let $\varphi:T_p(\ell_p)\to Y/W$ be a $p$-complete contraction. Given a finite-dimensional subspace $E\subset T_p(\ell_p)$ and $\varepsilon>0$, by the trace duality rigidity of $\ell_p$, there exists a finite-dimensional subspace $F\subset\mathcal{K}_p(\ell_p)$ such that $E = F^*$. We consider the localization $\varphi|_E:E\to Y/W$. Taking the transpose yields
\[
(\varphi|_E)^*: (Y/W)^* \cong W^\perp \to E^* \cong F \subset B_p(\ell_p).
\]
By the assumption that $B_p(\ell_p)$ is $p$-injective, this map can be extended to a $p$-completely bounded map $\Phi:Y^*\to B_p(\ell_p)$ with $\|\Phi\|_{\pcb}\le 1$. Transposing again and restricting to $E$, we obtain $\Phi^*|_E:E\to Y^{**}$. By the local reflexivity of $\ell_p$ (which transfers to $T_p(\ell_p)$), $\Phi^*|_E$ can be viewed as a map $\tilde\varphi:E\to Y$ satisfying $q\tilde\varphi=\varphi|_E$ and $\|\tilde\varphi\|_{\pcb}\le 1+\varepsilon$. Therefore, $T_p(\ell_p)$ has the $p$-LLP.

\textbf{Direction 2: $T_p(\ell_p)$ has the $p$-LLP $\Rightarrow B_p(\ell_p)$ is $p$-injective.}
Let $W_0\subseteq W$ be $p$-operator spaces and let $\psi:W_0\to B_p(\ell_p)$ be a $p$-completely bounded map. By the $p$-approximation property of $\mathcal{K}_p(\ell_p)$, for any finite-dimensional subspace $E\subset W_0$, we can choose a finite-rank projection $P_n: B_p(\ell_p)\to B_p(\ell_p^n)$. Let $\psi_E = P_n \circ \psi|_E: E\to B_p(\ell_p^n)$. Since the finite-dimensional space $B_p(\ell_p^n)$ is $p$-injective (Proposition~\ref{prop:fd-injective}), $\psi_E$ can be extended to $\tilde\psi_E:W\to B_p(\ell_p^n)$. By the $p$-LLP of $T_p(\ell_p)$, the norm of this extension can be calibrated such that $\|\tilde\psi_E\|_{\pcb}\le 1+\varepsilon$. Finally, utilizing the weak$^*$ compactness of $B_p(\ell_p)$, taking the limit of the net over finite-dimensional subspaces yields a global extension $\tilde\psi:W\to B_p(\ell_p)$ satisfying $\tilde\psi|_{W_0}=\psi$ and $\|\tilde\psi\|_{\pcb}\le 1+\varepsilon$. Hence, $B_p(\ell_p)$ is $p$-injective.
\end{proof}

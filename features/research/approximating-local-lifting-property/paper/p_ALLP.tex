...
As above, $\mathcal{P}_n$ is a $p$-complete quotient map (because its adjoint $B(\ell_p^n)\to B(\ell_p)$, $T\mapsto \iota_n T p_n$, is a $p$-complete isometry).
By Lemma~\ref{lem:Pn-approx-identity}, $j_n\mathcal{P}_n|_E\to\mathrm{id}_E$ in norm.

We take $\eta=\varepsilon/2>0$ and choose $n$ such that
\[
        \|j_n\mathcal{P}_n|_E-\mathrm{id}_E\|<\min\{1,\eta/2\}.
\]
This shows that $\mathcal{P}_n|_E$ is one-to-one: if $x\in E$ satisfies $\mathcal{P}_nx=0$, then
\[
        \|x\|=\|x-j_n\mathcal{P}_nx\|
        \leq \|j_n\mathcal{P}_n|_E-\mathrm{id}_E\|\,\|x\|,
\]
so $x=0$.
We define
\[
...
\[
        ...
        =\|\varphi j_n\mathcal{P}_n|_E-\varphi|_E\|<\eta/2 .
\]
...
Applying this to the $p$-complete contraction $\varphi_n:\mathcal{N}(\ell_p^n)\to Y/W$ and the finite-dimensional subspace $\mathcal{P}_n(E)\subseteq \mathcal{N}(\ell_p^n)$ with tolerance $\eta/2$, there exists a $p$-completely bounded map
$\widetilde\psi:\mathcal{N}(\ell_p^n)\to Y$ such that $\|\widetilde\psi\|_{\pcb}\leq 1$ and
\[
        \|q\widetilde\psi|_{\mathcal{P}_n(E)}-\varphi_n|_{\mathcal{P}_n(E)}\|<\frac{\eta}{2\|\mathcal{P}_n|_E\|}.
\]
We define
\[

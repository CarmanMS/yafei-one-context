...
{\bf Abstract:} For $1<p<\infty$, we investigate local lifting properties and injectivity for $p$-operator spaces. We prove that for any $p$-operator space, the $p$-approximately local lifting property is equivalent to the $p$-local lifting property. For dual $p$-operator spaces, we show that $p$-approximate injectivity coincides with $p$-injectivity. In particular,  we relate the $p$-injectivity of $B(\ell_p)$ and the $p$-local lifting property of $\mathcal{N}(\ell_p)$ to their finite-dimensional counterparts, and show that every $n$-dimensional $p$-operator space has the $n$-$p$-LLP and  $n$-$p$-injectivity together.
\par
{\bf Keywords:} $p$-operator space, $p$-approximately local lifting property,
$p$-approximate injectivity.
\par
...
In this paper, we will study the local lifting properties and injectivity for $p$-operator spaces. Let us recall from the classical operator space theory \cite{KyeRuan1999} that an operator space $V$ is said to have the $\lambda$-local lifting property ($\lambda$-LLP) if given any operator spaces $W \subseteq Y$, any complete contraction $\varphi: V \to Y/W$, any finite-dimensional subspace $E \subseteq V$ and $\varepsilon>0$, there exists a completely bounded map $\tilde{\varphi}: E \to Y$ such that
\[
q\circ \tilde{\varphi}=\varphi|_E
\qquad\text{and}\qquad
\|\tilde{\varphi}\|_{\mathrm{cb}}< \lambda+\varepsilon,
\]
where $q: Y \to Y/W$ is the complete quotient map. If $\lambda = 1$, we simply say that $V$ has the LLP. An operator space $V$ is \emph{$\lambda$-injective} if given any operator spaces $W\subseteq Y$, and any completely bounded map $\psi:W\to V$, there is an extension $\widetilde{\psi}:Y\to V$ such that
\[
\widetilde{\psi}|_{W}=\psi,\qquad
\|\widetilde{\psi}\|_{\mathrm{cb}}\leq\lambda\|\psi\|_{\mathrm{cb}}.
\]
For $\lambda=1$, we say that $V$ is injective. The local lifting property is a natural non-commutative analogue of the Banach space local lifting property, but it exhibits much richer structures. Kye and Ruan \cite{KyeRuan1999} proved that an operator space $V$ has the $\lambda$-LLP if and only if its dual $V^*$ is $\lambda$-injective. This theorem reveals a deep connection between the local lifting property and injectivity.

Let $1<p<\infty$. Following Pisier \cite{Pisier1990}, Le Merdy \cite{LeMerdy1996} and Daws \cite{Daws2007} presented the abstract framework for $p$-operator spaces, which generalizes classical operator spaces (the $p=2$ case) to the $L^p$ setting. The corresponding theory of $p$-operator spaces has since been developed in considerable detail. The $p$-approximation property was investigated by An et al.\ \cite{AnLeeRuan2010}. Lee \cite{Lee2014} studied the local structure of $p$-operator spaces through conditions $C_p$, $C'_p$, and $C''_p$. Zhao and Dong \cite{ZD14} introduced $p$-mapping spaces and applied them to the study of $p$-local reflexivity. Moreover, if $V$ is a $p$-operator space, then its dual space $V^* = \CBp(V, \mathbb{C})$ is naturally a $p$-operator space and agrees isometrically with the Banach dual (see \cite{Daws2007}).

In the study of local properties, it is natural to consider the approximate versions of these properties. For classical operator spaces, Dong \cite{Dong2007} introduced the approximately local lifting property (ALLP) and approximate injectivity, and showed that for an operator space $V$,
\[
\begin{aligned}
...
\end{aligned}
\]
This motivates us to investigate whether similar equivalence holds in the category of $p$-operator spaces. However, the proof of the equivalence ($V\text{ has the } \lambda\text{-LLP} \Longleftrightarrow V^*\text{ is } \lambda \text{-injective}$) does not extend to $p$-operator spaces, since the Arveson--Wittstock extension theorem fails in general \cite{Lee2015}. 
In Section 2  we establish the equivalence between the $p$-ALLP and the $p$-LLP for any $p$-operator space (Theorem~\ref{thm:main}); and in Section 3 we prove that for dual $p$-operator spaces, $p$-approximate injectivity is equivalent to $p$-injectivity (Theorem~\ref{thm:dual-exact-approx});  in Section 4 we relate the $p$-injectivity of $B(\ell_p)$ and the $p$-local lifting property of $\mathcal{N}(\ell_p)$ to their finite-dimensional counterparts; finally, in Section 5 we apply these results to finite-dimensional $p$-operator spaces and show that any $n$-dimensional $p$-operator space has the $n$-$p$-LLP and $n$-$p$-injectivity together.

\section{$p$-LLP and $p$-ALLP}
...
Let $\lambda\geq 1$.  A $p$-operator space $V$ has the
\emph{$\lambda$-$p$-local lifting property} ($\lambda$-$p$-LLP) if given any $p$-operator spaces $W\subseteq Y$, any $p$-complete contraction
...
Let $\lambda\geq 1$.  A $p$-operator space $V$ has the
\emph{$\lambda$-$p$-approximately local lifting property} ($\lambda$-$p$-ALLP) if
given any $p$-operator spaces $W\subseteq Y$, any $p$-complete contraction
...
A completely analogous result can be proved for the $\lambda$-versions by the same arguments, but for simplicity of notation we restrict ourselves to the case $\lambda=1$ in Section 2, Section 3 and Section 4. From the definitions, for every $p$-operator space $V$ we have
\(
        p\hbox{-LP}
        \Longrightarrow
        p\hbox{-LLP}
        ...
        p\hbox{-ALLP}.
\)
To show that the $p$-LLP and the $p$-ALLP are equivalent, we first prove two quantitative estimates for finite-dimensional $p$-operator spaces. 
...
Let $V$ be a $p$-operator space.  Then $V$ has the
$p$-ALLP if and only if $V$ has the $p$-LLP.
\end{theorem}

\begin{proof}
It is clear that the $p$-LLP implies the $p$-ALLP.

Conversely, we assume that $V$ has the $p$-ALLP.  We suppose that $W\subseteq Y$ are $p$-operator spaces. We denote by $q:Y\to Y/W$ the $p$-complete quotient map.  Let $\varphi:V\to Y/W$ be a $p$-complete contraction, $E\subseteq V$ be finite-dimensional, and $\varepsilon>0$.  If $\varphi|_E=0$, then the zero operator $0:E\to Y$ satisfies $q\circ 0=\varphi|_E$. This operator is the required lifting.  We may therefore assume that $\varphi|_E\neq 0$.
...
\]
By the $p$-ALLP applied to $\varphi$ with $\varepsilon/2$, there
is a net $\{w_\alpha\}$ in $\CBp(E,Y)$ with
\[
        \|w_\alpha\|_{\pcb}<1+\frac{\varepsilon}{2}.
\]
and $qw_\alpha\to \varphi|_E$ in the point-norm topology on $Y/W$.  Since $E$ is
finite-dimensional, the point-norm convergence is equivalent to the operator norm
convergence on $E$, so we may choose $\varphi_0=w_\alpha$ for some $\alpha$ satisfying
\[
...
\]
This shows that $V$ has the $p$-LLP.
\end{proof}

\begin{remark}\label{rem:main-on-Lp}
The same argument shows that for every $p$-operator space $V$ and every
$\lambda\geq 1$, the $\lambda$-$p$-ALLP and the $\lambda$-$p$-LLP are equivalent
whenever in those definitions $W\subseteq Y$ are $p$-operator spaces on $L_p$ spaces.
\end{remark}

\begin{proposition}\label{prop:complemented}
Let $V$ be a $p$-operator space and let $P:V\to V$ be a $p$-completely contractive
projection.  We denote by $V_0$ the subspace $P(V)\subseteq V$, with the
$p$-operator space structure induced by $V$.
\begin{enumerate}
        \item[\rm(i)] If $V$ has the $p$-LLP, then $V_0$ has the
                $p$-LLP;
        \item[\rm(ii)] If $V$ has the $p$-ALLP, then $V_0$ has the
                $p$-ALLP.
\end{enumerate}
...
\begin{proof}
It follows from Theorem~\ref{thm:main} that we only need to prove
{\rm(i)}.  Suppose that $W\subseteq Y$ are $p$-operator spaces and denote by $q:Y\to Y/W$ the $p$-complete quotient map.  Let $\varphi:V_0\to Y/W$ be a $p$-complete contraction and $E\subseteq V_0$ be finite-dimensional.  Applying the $p$-LLP of $V$ to the finite-dimensional subspace
$E\subseteq V$ and the $p$-complete contraction $\varphi P: V\to Y/W$, for any $\varepsilon>0$ we obtain
$\widetilde{\varphi}:E\to Y$ such that
\[
        q\widetilde{\varphi}=\varphi P|_E=\varphi|_{E}
\]
and
\[
        \|\widetilde{\varphi}\|_{\pcb}<1+\varepsilon.
\]
This implies that $V_{0}$ has the $p$-LLP.
\end{proof}

\section{$p$-Injectivity and $p$-Approximate Injectivity}
...
Let $\lambda\geq 1$.  A $p$-operator space $V$ is
\emph{$\lambda$-$p$-approximately injective} if given $p$-operator spaces $W_0\subseteq W$, and any $p$-completely bounded map $\varphi:W_0\to V$, for any $\varepsilon>0$
there exists a net of $p$-completely bounded
...
$W_0$.
For $\lambda=1$, we say that $V$ is $p$-approximately injective.
\end{definition}
...
As in Section 2,  for simplicity of notation we restrict ourselves to the case $\lambda=1$.
...
$p$-operator space structure induced by $V$.
\begin{enumerate}
        \item[\rm(i)] If $V$ is $p$-injective, then $V_0$ is
                $p$-injective;
        \item[\rm(ii)] If $V$ is $p$-approximately injective, then $V_0$
                is $p$-approximately injective.
\end{enumerate}
...
$p$-completely bounded map $\varphi:W_0\to V$.  By $p$-approximate injectivity of $V$,
for any $\varepsilon>0$ there is a net
$\widetilde{\varphi}_\alpha:W\to V$ such that
\[
        \|\widetilde{\varphi}_\alpha\|_{\pcb}
        \leq(1+\varepsilon)\|\varphi\|_{\pcb}
\]
...
and $(P\widetilde{\varphi}_\alpha)|_{W_0}\to P\varphi=\varphi$ in the
...
\[
        ...
        \leq (1+\varepsilon)\|\varphi\|_{\pcb}.
\]
This implies that $V_{0}$ is $p$-approximately injective.
\end{proof}

A fundamental question is how these injectivity conditions behave under duality. The following theorem shows that for the dual $p$-operator space $V^*$, $p$-approximate injectivity coincides with $p$-injectivity.

\begin{theorem}\label{thm:dual-exact-approx}
Let $V$ be a $p$-operator space.  Then $V^*$ is
$p$-approximately injective if and only if $V^*$ is
$p$-injective.
\end{theorem}

\begin{proof}
It is clear that $p$-injectivity implies $p$-approximate injectivity.

For the converse, let $W_0\subseteq W$ be $p$-operator spaces, and $\varphi:W_0\to V^*$ be $p$-completely bounded.
...
$\delta_2\leq\delta_1$.  By $p$-approximate injectivity of $V^*$,
...
$\|\widetilde{\varphi}_d\|_{\pcb}\leq 2\|\varphi\|_{\pcb}$, so the net
lies in the closed ball
\[
        \mathcal{K}_M=\{S\in \CBp(W,V^*):\|S\|_{\pcb}\leq M\},
        \qquad M=2\|\varphi\|_{\pcb}.
\]
...
\[
M_m(V^*)=\CBp(V,\mathbb{M}_m)\cong (\mathcal{N}_m^p\otimesp V)^*,
\]
where $\mathcal{N}_m^p=\mathcal{N}(\ell_p^m)$ is the space of nuclear operators on
$\ell_p^m$.
...
\[
        ...
        \leq(1+\varepsilon_i)\|\varphi\|_{\pcb}.
\]
Passing to a subnet with $\varepsilon_i\to 0$, we obtain
\[
        \|T_m(w)\|\leq\liminf_i\|(\widetilde{\varphi}_{d_i})_m(w)\|
        \leq\liminf_i(1+\varepsilon_i)\|\varphi\|_{\pcb}
        =\|\varphi\|_{\pcb}.
\]
We take the supremum over $m$ and over $w$ in the unit ball of $M_m(W)$
to obtain $\|T\|_{\pcb}\leq\|\varphi\|_{\pcb}$, so $V^*$ is
$p$-injective.
\end{proof}



\section{$p$-Injectivity of $B(\ell_p)$ and $p$-LLP of $\mathcal{N}(\ell_p)$}

In this section, we relate the $p$-injectivity of $B(\ell_p)$ and the $p$-local lifting property of $\mathcal{N}(\ell_p)$ to their finite-dimensional counterparts. We first fix some standard notation for finite-dimensional approximations.
...
corresponding projection onto the first $n$ coordinates.
...
$\mathcal{P}_n j_n=\mathrm{id}_{\mathcal{N}(\ell_p^n)}$, and $j_n\mathcal{P}_n$ is a $p$-completely contractive projection of $\mathcal{N}(\ell_p)$ onto
$j_n(\mathcal{N}(\ell_p^n))$.
For $\omega\in \mathcal{N}(\ell_p)$ we have $P_n\omega P_n=j_n(\mathcal{P}_n(\omega))$ as
operators on $\ell_p$.
Furthermore, the subspace $\bigcup_{m=1}^\infty j_m(\mathcal{N}(\ell_p^m))$ is norm dense in $\mathcal{N}(\ell_p)$.

\begin{lemma}\label{lem:Qn-approx-identity}
Let $1<p<\infty$. For each $n\in\mathbb{N}$, let $Q_n:B(\ell_p)\to B(\ell_p)$ be given by $Q_n(T)=P_nTP_n$.
...
Let $q:Y\to Y/W$ denote the $p$-complete quotient map, and $\varphi:E\to Y/W$ be a $p$-complete contraction.
...
in the point-weak$^*$ topology of $\CBp(E,Y^{**})$, where $\kappa_Y: Y\hookrightarrow Y^{**}$.
...
Let $1<p<\infty$. Then $j_n\mathcal{P}_n\to \mathrm{id}_{\mathcal{N}(\ell_p)}$ in the point-norm topology.
In particular, if $E\subseteq \mathcal{N}(\ell_p)$ is a finite-dimensional subspace, then $j_n\mathcal{P}_n|_E\to\mathrm{id}_E$ in the norm topology.
\end{lemma}
...
\[
        ...
        \leq 2\|x-\tau\|<\delta.
\]
Hence $j_n\mathcal{P}_n\to \mathrm{id}_{\mathcal{N}(\ell_p)}$ in the point-norm topology.
Since $E$ is finite-dimensional, $j_n\mathcal{P}_n|_E\to\mathrm{id}_E$ in the norm topology.
\end{proof}
...
$\varphi:\mathcal{N}(\ell_p)\to Y/W$ be a $p$-complete contraction, 
$E\subseteq \mathcal{N}(\ell_p)$ be finite-dimensional and $\varepsilon>0$.

By Lemma~\ref{lem:Pn-approx-identity}, $j_n\mathcal{P}_n|_E\to\mathrm{id}_E$ in norm. We may assume that $E\neq\{0\}$ (otherwise the condition is trivial).
...
\]
Since $\mathcal{P}_n$ is a $p$-complete quotient map, $\|\widehat\psi\|_{\pcb}\leq 1$, and we have
\begin{align*}
...
\section{$n$-$p$-LLP and $n$-$p$-Injectivity}

In this section, we show that any finite-dimensional $p$-operator space $V$ with $\dim V=n$ has the $n$-$p$-LLP and $n$-$p$-injectivity together.


\begin{proposition}\label{prop:fd-equivalence}
Let $V$ be a finite-dimensional $p$-operator space with $n=\dim V\geq 1$.
Then $V$ has the $n$-$p$-LP, and hence the $n$-$p$-LLP.
\end{proposition}

\begin{proof}
 Given $\varepsilon>0$, we choose $\eta>0$ with
...
We assume that $\psi\neq 0$, and define $\psi_1=\psi/\|\psi\|_{\pcb}$, so $\|\psi_1\|_{\pcb}=1$.
...
Indiana Univ. Math. J. \textbf{26} (1977), 443--446.
...
Illinois J. Math. \textbf{58} (2014), 1071--1091.

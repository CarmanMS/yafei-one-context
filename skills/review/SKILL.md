---
name: review
description: 多智能体协作评审技术方案，支持双 Agent 对弈和多角色委员会两种模式。触发：方案评审、review、技术方案评审。
---
...
| 方式 | 触发条件 | 特点 | 适用场景 |
|------|---------|------|---------|
| **Subagent 并发（默认）** | 直接调用或 `--team=false` | 高效并行，结果汇总后展示 | Committee 模式、快速评审 |
| **Teams 可视化** | `--team` 参数 | 分屏实时显示，可观察过程 | Duel 模式、需要观察对弈过程 |

**交互式选择**（当未指定 `--team` 且未指定 `mode` 时）：

```
? 选择评审执行方式：
  > 快速评审（默认）- 后台并行执行，效率高
    可视化评审（Teams）- 分屏显示，可观察过程
```

**执行方式与评审模式匹配建议**：

```python
# Committee 模式 → 推荐 Subagent 并发
if mode == "committee":
    # 并行启动多个角色 subagent
    # 汇总结果后修改方案
    use_parallel_subagents()

# Duel 模式 → 推荐 Teams 可视化（可选）
if mode == "duel":
    # 挑战者-辩护者需要持续对话
    # Teams 分屏可以实时观察对弈
    if team_mode:
        use_teams_visualization()
    else:
        use_sequential_subagents()
```
...
- **数据源合规**：严格遵守 `knowledge/README.md` 数据源规则，只搜索 `knowledge/internal/` 和 `knowledge/external/`
...
def load_knowledge_context(role, solution_content):
    ...
    # 3. 在 knowledge/internal/ 和 knowledge/external/ 中搜索
    #    使用 Grep 工具，搜索范围严格限定
    hits = grep(search_terms, paths=[
        "knowledge/internal/",
        "knowledge/external/"
    ])
...
**执行时机**：角色评审启动前，与角色 subagent 创建并行执行

**上下文注入格式**：
```markdown
## 参考资料（来自知识库）

以下是与本方案相关的内部知识，仅供参考，不要求强制引用：
...
- ❌ 回滚方案、灰度机制 → SRE 角色
- ❌ 要求设计文档写具体实现代码 → 设计文档阶段不适用
- ❌ 边界条件处理的完整性 → 架构师角色

**正确的问题示例 ✅**：
| 问题 | 优先级 | 定级依据 |
|------|-------|---------|
| 方案引入了3个新依赖，建议评估必要性 | P1 | 增加学习成本和维护负担，影响交付速度 |
| 步骤5-8可以合并为一个批量操作，减少实现复杂度 | P1 | 当前设计增加了不必要的实现步骤 |
| 预期工期3天过于乐观，仅数据迁移部分就需要2天 | P1 | 存在交付延期风险 |
| 建议用现成的 SDK 替代自研组件 | P2 | 能减少约1天开发时间 |

**错误的问题示例 ❌**（这些不应由开发者角色提出）：
| 问题 | 为什么错误 | 应属于的角色 |
|------|----------|------------|
| 缺少具体实现代码 | 设计文档阶段不应要求代码 | 不适用 |
| 单元测试覆盖不足 | 测试质量不是开发者的评审维度 | QA |
| 监控指标定义不完整 | 可观测性是 SRE 的关注点 | SRE |
| 缺少性能影响评估 | 性能评估是架构/SRE 的职责 | 架构师/SRE |
| 缺少回滚方案 | 运维容错是 SRE 的职责 | SRE |
...
# Teams 可视化模式（分屏显示对弈过程）
/review docs/api_design.md --mode duel --team

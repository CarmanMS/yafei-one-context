...
| C2 | frontmatter 最小三字段 | ERROR | ✅ | `type` / `status` / `created` 必填 |
...
| C7 | 标签两级且以领域开头 | ERROR | ✅ | `#科研/...` `#教学/...` `#家庭/...`；删除非法标签 |
| C8 | 笔记至少 1 个 wikilink | WARN | ❌ | 排除 Inbox 随手记 |

**严重级别**：
- ERROR：必须修复，`--fix` 自动处理（**C1 除外**，重命名为中文需语义理解，留给人工）
...
| 规则 | 修复动作 |
|------|---------|
| C2 | 补全 `type` / `status` / `created`（缺失字段用合理默认值，created 用文件 mtime 或 today） |
| C4 | 将跨域归档的笔记移回原领域 `_archive/`，或仅设 `status: archived` |
| C5 | 将顶层非法目录整体移入最近领域目录（`git mv`） |
| C7 | 删除非法标签（保留 `#领域/子主题` 形式） |
...
移动文件时：
1. 更新 frontmatter（如补 `type` / `status` / `created`）
2. `git mv` 文件进最近领域
3. 更新源和目标 `_MOC.md`
4. 扫反向链接（`[[旧路径]]`），逐一 PATCH 更新
...
- `--fix` 仅自动修复 ERROR 项（C1 除外，需人工）
- 修复前必须展示预览
- C5 修复涉及 git mv + `_MOC.md` 更新 + 反向链接扫描，必须完整执行
- C7 修复只删除非法标签，不添加新标签
